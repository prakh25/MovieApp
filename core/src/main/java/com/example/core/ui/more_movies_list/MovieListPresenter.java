...
import com.example.core.model.more_movie_list.box_office.RevenueComparator;
...
public class MovieListPresenter extends BasePresenter<MovieListContract.MoreView>
    ...
    private MovieStatus findInRealmMovieStatus(Realm realm, Integer id) {
        return realm.where(MovieStatus.class).equalTo(Constants.FIELD_MOVIE_ID, id)
                .findFirst();
    }

    private UserRating findInRealmUserRating(Realm realm, Integer id) {
        return realm.where(UserRating.class).equalTo(Constants.FIELD_MOVIE_ID, id)
                .findFirst();
    }
